...
const char *TAG = "http_gettask";
int16_t *audio = NULL; // = (int16_t *)malloc(960 * 2 * sizeof(int16_t));
char *buff = NULL;
char *base_message_serialized = NULL;
...
void http_gettask(void *pvParameters) {
  ...
  while (true) {
    ...
    inet_pton(AF_INET, CONFIG_SNAPCAST_SERVER_HOST,
    ...
    while (true) {
      ...
      struct timeval to;
      to.tv_sec = CONFIG_WEBSOCKET_SERVER_TIMEOUT_SEC;
      to.tv_usec = CONFIG_WEBSOCKET_SERVER_TIMEOUT_USEC;
      ...
      if (retval) {
        ...
        ESP_LOGI(TAG, "Socket timeout after %d sec / %u ms", to.tv_sec,
      ...
      case SNAPCAST_MESSAGE_CODEC_HEADER:
        result = codec_header_message_deserialize(&codec_header_message, start,
                                                  size);
        if (result) {
        ...
        ESP_LOGI(TAG, "Codec : %s , Size: %d ", codec_header_message.codec,
                 size);
      ...
      case SNAPCAST_MESSAGE_WIRE_CHUNK: {
        ...
        case PCM:
          ...
          if ((chunk_res =
                   write_ringbuf(timestampSize, 3 * sizeof(uint32_t))) != 12) {
            ESP_LOGI(TAG, "Error writing timestamp to ring buffer: %d",
                     chunk_res);
          }
      ...
      case SNAPCAST_MESSAGE_TIME:
        ...
        // timeravg(&tavg,&ctx.tdif);
        // ESP_LOGI(TAG, "Tclienctx.tdif :% 11ld.%03ld ", ctx.tdif.tv_sec ,
        // ctx.tdif.tv_usec/1000);
        char retbuf[10];
        retbuf[0] = 5;
        retbuf[1] = 5;
        uint32_t usec = ctx.tdif.tv_usec;
        uint32_t uavg = ctx.tavg.tv_usec;
        // ESP_LOGI(TAG, "Tclienctx.tdif : return value %d ",uavg);
      ...
      if (tv1.tv_sec >= 1) {
        ...
        result = time_message_serialize(&time_message, buff,
